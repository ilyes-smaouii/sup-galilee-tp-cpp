...
pilelivre::PileLivre_v2 first_day() {
  pilelivre::PileLivre_v2 res{};
  ...
  res.add_friend(
      "Jack",
      "James"); // edges are oriented, this is necessary if we don't want the
                // call to rm_friend() below to throw an exception
  res.add_msg_private("Jack", "James", "t'es qui ???");
  res.rm_friend("Jack", "James");
  return res;
}

void display_pilelivre_state(const pilelivre::PileLivre_v2 &pl) {
  ...
  for (const auto &datum_ptr : pl.m_data) {
    ...
    if (person_ptr != NULL) {
      ...
      std::cout << std::format(
                       "(name : {}, actions count : {}, friends count {})",
                       person_ptr->m_name, person_ptr->m_action.size(),
                       person_ptr->m_friends.size())
                << std::endl;
    }
    if (friend_add_ptr != NULL) {
      std::cout << "[friend_add]" << std::endl;
      std::cout << std::format("(Added {})", friend_add_ptr->m_person->m_name)
                << std::endl;
    }
    if (friend_remove_ptr != NULL) {
      std::cout << "[friend_remove]" << std::endl;
      std::cout << std::format("(Removed {})",
                               friend_remove_ptr->m_person->m_name)
                << std::endl;
    }
    if (msg_pub_ptr != NULL) {
      std::cout << "[msg_pub]" << std::endl;
      std::cout << std::format("(Msg content : \"{}\")", msg_pub_ptr->m_msg)
                << std::endl;
    }
    if (msg_priv_ptr != NULL) {
      std::cout << "[msg_priv]" << std::endl;
      std::cout << std::format("(Msg dest : {}, msg content : \"{}\")",
                               msg_priv_ptr->m_dest->m_name,
                               msg_priv_ptr->m_msg)
                << std::endl;
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  // std::cout << "res.m_counter_friend : " << res.m_counter_friend <<
  // std::endl;
