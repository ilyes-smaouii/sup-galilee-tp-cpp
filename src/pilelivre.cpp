...
namespace pilelivre {
  ...
  void PileLivre_v1::add_friend(std::string name, std::string _friend) {
    ...
    // remove from friends lists, and remove edge
    person_ptr->m_friends.push_back(friend_person_ptr);
    friend_person_ptr->m_friends.push_back(person_ptr);
    m_content.add_edge(person_ptr->m_name, friend_person_ptr->m_name, true);
    // Add action to actions list, + add FriendRemove to m_data
  ...
  void PileLivre_v1::rm_friend(std::string name, std::string friend_no_more) {
    ...
    friend_person_ptr->m_friends.remove(person_ptr);
    m_content.rm_edge(person_ptr->m_name, friend_person_ptr->m_name);
    // Add action to actions list, + add FriendRemove to m_data
  ...
  PileLivre_v2::~PileLivre_v2() {
    for (auto data_ptr : m_data) {
      delete data_ptr;
    }
  }
  
  void PileLivre_v2::add_person(std::string name) {
    auto new_person_ptr = new Person(name);
    m_data.push_back(new_person_ptr);
    auto node_id = name;
    auto node_data = new_person_ptr;
  ...
  void PileLivre_v2::add_msg_public(std::string name, std::string msg) {
    auto person_node_iter = m_content.m_nodes.find(name);
    if (person_node_iter == m_content.m_nodes.end()) {
      throw std::runtime_error(std::format("PileLivre_v2::add_msg_public error : node with name {} not found in Graph nodes !", name));
    }
    auto person_ptr = dynamic_cast<Person*>(person_node_iter->second.m_data);
    if (person_ptr == NULL) {
      throw std::runtime_error(std::format("PileLivre_v2::add_msg_public error : found node with name {}, but it's not a person !", name));
    }
    auto new_msg_ptr = new MsgPublic(msg);
    person_ptr->add_action(new_msg_ptr);
    m_data.push_back(new_msg_ptr);
  ...
  void PileLivre_v2::add_msg_private(std::string writer, std::string reader, std::string msg) {
    auto writer_node_iter = m_content.m_nodes.find(writer);
    if (writer_node_iter == m_content.m_nodes.end()) {
      throw std::runtime_error(std::format("PileLivre_v2::add_msg_private error : writer node with name {} not found in Graph nodes !", writer));
    }
    auto writer_person_ptr = dynamic_cast<Person*>(writer_node_iter->second.m_data);
    if (writer_person_ptr == NULL) {
      throw std::runtime_error(std::format("PileLivre_v2::add_msg_private error : found writer node with name {}, but it's not a person !", writer));
    }
    auto reader_node_iter = m_content.m_nodes.find(reader);
    if (reader_node_iter == m_content.m_nodes.end()) {
      throw std::runtime_error(std::format("PileLivre_v2::add_msg_private error : reader node with name {} not found in Graph nodes !", reader));
    }
    auto reader_person_ptr = dynamic_cast<Person*>(reader_node_iter->second.m_data);
    if (reader_person_ptr == NULL) {
      throw std::runtime_error(std::format("PileLivre_v2::add_msg_private error : found reader node with name {}, but it's not a person !", reader));
    }
    auto new_msg_ptr = new MsgPrivate(msg, reader_person_ptr);
    writer_person_ptr->add_action(new_msg_ptr);
    m_data.push_back(new_msg_ptr);
  ...
  void PileLivre_v2::add_cat(std::string name, std::string url) {
    auto person_node_iter = m_content.m_nodes.find(name);
    if (person_node_iter == m_content.m_nodes.end()) {
      throw std::runtime_error(std::format("PileLivre_v2::add_cat error : node with name {} not found in Graph nodes !", name));
    }
    auto person_ptr = dynamic_cast<Person*>(person_node_iter->second.m_data);
    if (person_ptr == NULL) {
      throw std::runtime_error(std::format("PileLivre_v2::add_cat error : found node with name {}, but it's not a person !", name));
    }
    auto new_cat_ptr = new CatPropaganda(url);
    person_ptr->add_action(new_cat_ptr);
    m_data.push_back(new_cat_ptr);
  ...
  void PileLivre_v2::add_friend(std::string name, std::string _friend) {
    // Find person
    auto person_node_iter = m_content.m_nodes.find(name);
    if (person_node_iter == m_content.m_nodes.end()) {
      throw std::runtime_error(std::format("PileLivre_v2::add_friend error : node with name {} not found in Graph nodes !", name));
    }
    auto person_ptr = dynamic_cast<Person*>(person_node_iter->second.m_data);
    if (person_ptr == NULL) {
      throw std::runtime_error(std::format("PileLivre_v2::add_friend error : found node with name {}, but it's not a person !", name));
    }
    // Find friend
    auto friend_node_iter = m_content.m_nodes.find(_friend);
    if (friend_node_iter == m_content.m_nodes.end()) {
      throw std::runtime_error(std::format("PileLivre_v2::add_friend error : friend node with name {} not found in Graph nodes !", _friend));
    }
    auto friend_person_ptr = dynamic_cast<Person*>(friend_node_iter->second.m_data);
    if (friend_person_ptr == NULL) {
      throw std::runtime_error(std::format("PileLivre_v2::add_friend error : found reader node with name {}, but it's not a person !", _friend));
    }
    // remove from friends lists, and remove edge
    person_ptr->m_friends.push_back(friend_person_ptr);
    friend_person_ptr->m_friends.push_back(person_ptr);
    m_content.add_edge(person_ptr->m_name, friend_person_ptr->m_name);
    // Add action to actions list, + add FriendRemove to m_data
    auto new_friend_add_ptr = new FriendAdd(friend_person_ptr);
    person_ptr->add_action(new_friend_add_ptr);
    m_data.push_back(new_friend_add_ptr);
  ...
  void PileLivre_v2::rm_friend(std::string name, std::string friend_no_more) {
    // Find person
    auto person_node_iter = m_content.m_nodes.find(name);
    if (person_node_iter == m_content.m_nodes.end()) {
      throw std::runtime_error(std::format("PileLivre_v2::rm_friend error : node with name {} not found in Graph nodes !", name));
    }
    auto person_ptr = dynamic_cast<Person*>(person_node_iter->second.m_data);
    if (person_ptr == NULL) {
      throw std::runtime_error(std::format("PileLivre_v2::rm_friend error : found node with name {}, but it's not a person !", name));
    }
    // Find friend
    auto friend_node_iter = m_content.m_nodes.find(friend_no_more);
    if (friend_node_iter == m_content.m_nodes.end()) {
      throw std::runtime_error(std::format("PileLivre_v2::rm_friend error : friend node with name {} not found in Graph nodes !", friend_no_more));
    }
    auto friend_person_ptr = dynamic_cast<Person*>(friend_node_iter->second.m_data);
    if (friend_person_ptr == NULL) {
      throw std::runtime_error(std::format("PileLivre_v2::rm_friend error : found reader node with name {}, but it's not a person !", friend_no_more));
    }
    // remove from friends lists, and remove edge
    person_ptr->m_friends.remove(friend_person_ptr);
    friend_person_ptr->m_friends.remove(person_ptr);
